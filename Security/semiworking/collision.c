...
int main(int argc, char *argv[]) {
    ...
    generate_digest(mess, mess_hash, 3,
                    hash_algo);// a 24 bit (3 byte)hash is generated here and stored in variable mess_hash using a particular hash algrithm hash_algo
    ...
    for (j = 0; j < 10000; j++) {
        ...
        match = -1, // match_repo may return actual index
                match = match_repo(repo, repo_index,
                                   rand_hash);// This statement compares the random String's hash with the repository storing all the generated strings' hash
